...
if(argc > 1)
	...
	cap = VideoCapture("MultipleColors.mp4"); 
...
void processImage()
{
	...
	Scalar balcolor[nColorsDetect] = {Scalar(0,0,255),
									  Scalar(0,255,0),
									  Scalar(255,0,0)};
	
	std::string objectName[3] = {"RED BALL", "YELLOW BALL", "BLUE BALL"};
	...
	for (int i = 0; i < nColorsDetect; i++)
	{
		...
		for (int j = 0; j < circles.size(); j++)
		{
			 ...
			 std::string text = objectName[i] + std::string(" here: ") + std::to_string(center.x)+std::string(",") + std::to_string(center.y);
			 putText(result, text, Point(center.x-10 ,center.y-10), FONT_HERSHEY_SIMPLEX, 1, balcolor[i], 2);
			 circle(result, center, 20, balcolor[i], 2);
		}
	}
}
